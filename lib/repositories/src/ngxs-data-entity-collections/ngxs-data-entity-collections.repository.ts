...
> extends AbstractRepository<C> implements EntityRepository<V, K, C> {
    ...
    public get ids$(): Observable<K[]> {
        return this.state$.pipe(map((value: C): K[] => value.ids));
    }

    @Computed()
    public get entities$(): Observable<EntityDictionary<K, V>> {
        return this.state$.pipe(map((value: C): EntityDictionary<K, V> => value.entities));
    }

    private get ctx(): EntityContext<V, K, C> {
        return ensureDataStateContext<C, StateContext<C>>(this.context as StateContext<C>);
    }
    ...
    protected setEntitiesState(state: C): void {
    ...
    private generateKeyMap(state: C): KeysDictionary<K> {
